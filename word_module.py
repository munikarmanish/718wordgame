...
def get_word_score(word, n, letter_values=SCRABBLE_LETTER_VALUES):
    ...
    wordlen = len(word)
    first_component = sum([letter_values.get(char, 0) for char in word.lower()])
...
def update_hand(hand, word):
    ...
    for char in word.lower():
        try:
            new_hand[char] = max(new_hand[char]-1, 0)
            if new_hand[char] == 0:
                new_hand.pop(char, None)
        except KeyError:
            pass
...
def is_valid_word(word, hand, word_list):
    ...
    is_in_wordlist = word in word_list
    if (not is_in_wordlist) and ('*' in word):
        is_in_wordlist = True in [word.replace('*', v) in word_list for v in VOWELS]
    ...
    while hand:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list):
                ...
                total_score += score
                print('"{word}" earned {score} points. Total: {total} points'.format(
                    word=word, score=score, total=total_score))
...
def substitute_hand(hand, letter, avail_letters=VOWELS + CONSONANTS):
    ...
    invalid_letters = list(new_hand.keys()) + [letter]
    new_letter = letter
    while new_letter in invalid_letters:
        new_letter = random.choice(avail_letters)
